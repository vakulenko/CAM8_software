...
procedure debugWriteLine (str : string);
var
  dtime : TDateTime;
begin
  dtime:= Time;
 writeln (debugFile, FormatDateTime('hh:nn:ss.zzz', dtime)+str);
 Flush(debugFile);
end;
...
x,y,x1:word;
begin
debugWriteLine ('posl.Execute call');
...
debugWriteLine ('posl.Execute end');
  mImageReady := true;
...
i:integer;
begin
debugWriteLine ('AD9822 call adr='+inttostr(adr)+' val='+inttostr(val));
...
Write_USB_Device_Buffer(FT_CAM8B,@FT_Out_Buffer,adress);
adress:=0;
debugWriteLine ('readframe comread');
comread;
...
var x : integer;
begin
debugWriteLine ('CameraSetOffset call offset='+inttostr(val));
 x:=abs(2*val);
 if val < 0 then x:=x+256;
 AD9822(6,x);                       //смещение AD9822
debugWriteLine ('CameraSetOffset end');
 Result :=true;
end;
...
debugWriteLine ('CameraConnect call');
...
debugWriteLine ('CameraConnect end, FT_flag='+BoolToStr(FT_flag,true)+' FT_OP_flag='+BoolToStr(FT_OP_flag,true)+' Result=IsConnected='+BoolToStr(IsConnected,true));
...
begin
debugWriteLine ('CameraDisconnect call');
 FT_OP_flag := true;
...
debugWriteLine ('CameraDisconnect end, Result=FT_OP_flag='+BoolToStr(FT_OP_flag,true));
 Result:= FT_OP_flag;
end;
...
begin
debugWriteLine ('CameraGetCameraState call, Result='+IntToStr(mCameraState));
 Result := mCameraState;
end;

procedure ExposureTimerTick;  stdcall;
begin
debugWriteLine ('ExposureTimerTick');
 ...
 HC595($cf);                //on +15V
 Write_USB_Device_Buffer(FT_CAM8B,@FT_OUT_Buffer,adress);
debugWriteLine ('ExposureTimerTick call readframe, mBin='+inttostr(mBin));
 readframe (mBin, 1000);
debugWriteLine ('ExposureTimerTick end');
 //mImageReady := true;
end;
...
begin
debugWriteLine ('Timer15VTick');
 KillTimer (0,Timer15V);
 adress:=0;
 HC595($4f);
 Write_USB_Device_Buffer(FT_CAM8B,@FT_OUT_Buffer,adress);
debugWriteLine ('Timer15VTick end');
end;

{Check camera connection, return bool result}
function CameraIsConnected () : WordBool; stdcall; export;
begin
debugWriteLine ('CameraIsConnected call, Result='+booltostr(IsConnected,true));
  Result := IsConnected;
end;

function CameraStartExposure (Bin,StartX,StartY,NumX,NumY : integer; Duration : double; light : WordBool) : WordBool; stdcall; export;
begin
debugWriteLine ('CameraStartExposure call, Bin='+inttostr(Bin)+' StartX='+inttostr(StartX)+' StartY='+inttostr(starty)+' NumX='+inttostr(numx)+' Numy='+inttostr(numy)+' Duration='+FloatToStr(duration)+' light='+booltostr(light,true));
 ...
 adress:=0;      // clearwin
 shift3;
debugWriteLine ('CameraStartExposure, start timers');
 Write_USB_Device_Buffer(FT_CAM8B,@FT_OUT_Buffer,adress);
 ...
 begin
  mCameraState := 3;
debugWriteLine ('CameraStartExposure, readframe call');
  readframe (mBin,round(Duration*1000));
  //mImageReady := true;
 end;
debugWriteLine ('CameraStartExposure, end');
 Result := true;
...
var x,y : integer;
begin
debugWriteLine ('CameraGetImageReady call');
 if (mImageReady) then
 begin
 //camera download ccd
 mCameraState := 4;
debugWriteLine ('CameraGetImageReady, frame=bufim');
...
debugWriteLine ('CameraGetImageReady end, result='+booltostr(mImageReady,true));
 Result := mImageReady;
end;
