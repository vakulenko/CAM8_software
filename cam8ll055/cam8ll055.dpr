// --------------------------------------------------------------------------------
// ASCOM Camera driver low-level interaction library for cam8_v0.55
// Edit Log:
...
var   IsConnected : boolean = false;        //переменная-флаг, отображает состояние соединения с камерой
      CanStopExposureNow : boolean = true;  //переменная-флаг, отображает, когда можно остановить экспозицию
      ...
      StopExposureTimer : integer;          //таймер для проверки переменной-флага CanStopExposureCount
      CheckCanStopExposureCount : integer =0;   //количество проверок переменной-флага CanStopExposureCount
      co: posl;                             //переменная для второго потока (чтение изображения)
      bufim:camera_image_type;              //буферный массив-изображение для операций
...
begin
 if (CanStopExposureNow) then begin
