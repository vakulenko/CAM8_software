...
Function Open_USB_Device(DevIndex:DWord;var FT:DWord) : FT_Result;
Function Open_USB_Device_By_Serial_Number(var FT:DWord;Serial_Number:string) : FT_Result;
...
Function Get_USB_Device_Status(var FT: DWord) : FT_Result;
...
Function Set_USB_Device_LatencyTimer(var FT: DWord;Latency : Byte) : FT_Result;
Function Get_USB_Device_BitMode(var BitMode:Byte) : FT_Result;
Function Set_USB_Device_BitMode(var FT: DWord; Mask, Enable:Byte) : FT_Result;
...
// IO Buffer Sizes
    FT_In_Buffer_Size = $1000000;//$800000; //6000;//$4000;//$800000;  // 64k
...
if Show_mess <> nil then Show_mess.Add('error: '+Str);
End;
...
If Result <> FT_OK then FT_Error_Report('GetFTDeviceCount',Result) else
 if Show_mess <> nil then SHow_mess.Add('Number of devices : '+inttostr(FT_Device_Count));
End;
...
If Result <> FT_OK then FT_Error_Report('GetFTDeviceSerialNo',Result) else
if Show_mess <> nil then Show_mess.Add('SerialNumber :'+FT_Device_String);
End;
...
If Result <> FT_OK then FT_Error_Report('GetFTDeviceDescription',Result) else
if Show_mess <> nil then Show_mess.Add('Description :'+FT_Device_String);
End;
...
If Result <> FT_OK then FT_Error_Report('FT_Open',Result) else
begin
FT:= FT_Handle;
if Show_mess <> nil then Show_mess.Add('Open :'+FT_Device_String);
end;
End;
...
Begin
Result :=  FT_Close(FT);
If Result <> FT_OK then FT_Error_Report('FT_Close',Result) else
if Show_mess <> nil then Show_mess.Add('Close :'+FT_Device_String);
End;
...
FT_IO_Status := FT_Read(FT,@FT_In_Buffer,Read_Count,@Read_Result);
...
Function Set_USB_Device_TimeOuts(var FT: DWord;ReadTimeOut,WriteTimeOut:DWord) : FT_Result;
Begin
Result :=  FT_SetTimeouts(FT,ReadTimeout,WriteTimeout);
...
Function Get_USB_Device_QueueStatus(var FT: DWord) : FT_Result;
Begin
Result :=  FT_GetQueueStatus(FT,@FT_Q_Bytes);
...
Function Get_USB_Device_Status(var FT: DWord) : FT_Result;
Begin
Result :=  FT_GetStatus(FT, @FT_Q_Bytes, @FT_TxQ_Bytes, @FT_Event_Status);
...
Function Set_USB_Device_LatencyTimer(var FT: DWord;Latency:Byte) : FT_Result;
Begin
Result :=  FT_SetLatencyTimer(FT, Latency);
...
Function Set_USB_Device_BitMode(var FT:DWord; Mask,Enable:Byte) : FT_Result ;
Begin
Result := FT_SetBitMode(FT,Mask,Enable);
If Result <> FT_OK then FT_Error_Report('FT_SetBitMode ',Result) else
if Show_mess <> nil then SHow_mess.Add('BitMode Ok');
End;
